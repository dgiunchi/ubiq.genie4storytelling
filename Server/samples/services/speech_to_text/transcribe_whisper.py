...
parser.add_argument("--dry_run", default=False, action='store_true', help="Launched from console.")
parser.add_argument("--oneshot", default=False, action='store_true', help="Create the wav of phrase_timeout seconds and then transcribe only one time")
parser.add_argument("--non_english", default=False, action='store_true', help="Don't use the english model.")
...
parser.add_argument("--output_name", type=str, default="file.wav")
...
def recognize_from_stdin():
    ...
    #temp_file = "./wavs/file.wav"
    temp_file = os.path.join(args.output_folder, args.output_name)
    label_file = os.path.join(args.output_folder, "transcript.txt")
        ...
        while not done:
            try:
                ...
                if not empty:
                    ...
                    audio_data = sr.AudioData(last_sample, 16000, 2) # source.SAMPLE_RATE=16000 , source.SAMPLE_WIDTH = 2
        ...
        while not done:
            try:
                ...
                if not empty:
                    ...
                    if phrase_time and phrase_time - now > timedelta(seconds=args.phrase_timeout):
                        last_sample = bytes()
                        phrase_complete = True
                        print("terminated")
                        # Read the transcription.
